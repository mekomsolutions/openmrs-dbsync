package org.openmrs.eip.app.db.sync.service.security;
...
import org.openmrs.eip.app.db.sync.utils.FileUtils;
import org.openmrs.eip.app.db.sync.config.EncryptionProperties;
...
public abstract class AbstractSecurityService {
    ...
    private void initKeyRing(final EncryptionProperties props) {
        try {
            ...
            throw new SyncException("Error while initiating key ring", e);
        }
    }
    ...
    private void addPublicKey(final byte[] keyFile) {
        try {
            ...
            throw new SyncException("Impossible to add key to key ring", e);
        }
    }
    ...
    protected String toString(ByteArrayOutputStream encryptedOutputStream) {
        try {
            ...
            throw new SyncException("Error while converting output stream to string", e);
        }
    }
}
