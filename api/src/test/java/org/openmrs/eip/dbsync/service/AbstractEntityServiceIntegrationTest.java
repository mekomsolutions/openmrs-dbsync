...
public class AbstractEntityServiceIntegrationTest extends BaseDbDrivenTest {
	...
	public void save_shouldSaveANewPatientRecordWhenThereIsAnExistingPersonRecord() {
		...
		Assertions.assertNotNull(personService.getModel(uuid));
		Assertions.assertNull(patientService.getModel(uuid));
		...
		Assertions.assertNotNull(patientService.getModel(uuid));
		Assertions.assertEquals(initialCount + 1, patientService.getAllModels().size());
	}
	...
	public void save_shouldSaveAUserWhereTheCreatorAndTheUserAreTheSame() {
		...
		Assertions.assertNull(userService.getModel(uuid));
		...
		Assertions.assertNotNull(userService.getModel(uuid));
		Assertions.assertEquals(initialCount + 1, userService.getAllModels().size());
	}
